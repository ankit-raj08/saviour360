  ...
  return (
    <>
      <header style={{
        ...
        background: scrolled&&!isAuthPage ? "rgba(245,240,232,0.96)" : "#080F1A",
        backdropFilter:"blur(20px) saturate(1.4)",
        borderBottom: scrolled&&!isAuthPage ? "1px solid rgba(13,21,32,0.07)" : "1px solid rgba(255,255,255,0.09)",
      }}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 2rem",height:"100%",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}} className="nav-desktop">
            <Link to="/signin" style={{padding:"8px 18px",fontFamily:"'Inter',sans-serif",fontSize:13,fontWeight:500,color:onDark?"rgba(255,255,255,0.8)":"rgba(13,21,32,0.5)",textDecoration:"none",transition:"color 0.2s"}}
              onMouseEnter={e=>e.currentTarget.style.color=onDark?"#FFFFFF":"#0D1520"}
              onMouseLeave={e=>e.currentTarget.style.color=onDark?"rgba(255,255,255,0.8)":"rgba(13,21,32,0.5)"}>
              Sign In
            </Link>
    ...
    <Link to={item.href||"#"} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      style={{
        ...
        color: isActive
          ...
          : h
            ? (light?"#FFFFFF":"#0D1520")
            : (light?"rgba(255,255,255,0.85)":"rgba(13,21,32,0.5)"),
...
function DropdownLink({item,light}){
  ...
  const isActive = item.href ? pathname.startsWith(item.href) : item.dropdown?.some(d => pathname.startsWith(d.href.split("#")[0]) && d.href.split("#")[0].length > 1);
  const labelColor = open||isActive?(light?"#FFFFFF":"#1A50A0"):(light?"rgba(255,255,255,0.85)":"rgba(13,21,32,0.5)");
  const borderStyle = isActive ? `2px solid ${light?"rgba(255,255,255,0.7)":"#1A50A0"}` : "2px solid transparent";
  return(
    <div style={{position:"relative"}} onMouseEnter={enter} onMouseLeave={leave}>
      <div style={{display:"flex",alignItems:"center",borderBottom:borderStyle}}>
        <Link
          to={item.href||"#"}
          style={{padding:"8px 4px 6px 16px",fontFamily:"'Inter',sans-serif",fontSize:13,fontWeight:isActive?600:500,color:labelColor,textDecoration:"none",transition:"color 0.2s",background:"none",border:"none",borderBottom:"none",cursor:"pointer"}}
        >
          {item.label}
        </Link>
        <span style={{padding:"8px 12px 6px 2px",cursor:"pointer",color:labelColor,display:"flex",alignItems:"center"}}>
      ...
      {item.dropdown?(
        <>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <Link to={item.href||"#"} onClick={onClose} style={{padding:"16px 0",fontFamily:"'Inter',sans-serif",fontSize:15,fontWeight:500,color:"rgba(255,255,255,0.8)",textDecoration:"none",flex:1}}>
              {item.label}
            </Link>
            <button onClick={()=>setOpen(o=>!o)} style={{background:"none",border:"none",cursor:"pointer",padding:"16px 0",color:"rgba(255,255,255,0.8)"}}>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{transform:open?"rotate(180deg)":"none",transition:"transform 0.2s",display:"block"}}>
