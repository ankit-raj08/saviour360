...
const toTestimonial = (doc) => ({
  name:    doc.name,
  role:    doc.role,
  company: doc.company,
  text:    doc.quote,
  avatar:  doc.avatarInitials || doc.name?.split(" ").map(w => w[0]).join("").slice(0,2).toUpperCase() || "?",
});

const TESTIMONIALS_ENDPOINT = "/api/testimonials?where[active][equals]=true&sort=order&limit=20";

export default function TestimonialsSection() {
  const { data: TESTIMONIALS } = useCMS(TESTIMONIALS_ENDPOINT, FALLBACK_TESTIMONIALS, toTestimonial);
    ...
    <section style={{ background: "#F5F0E8", padding: "72px 0" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 2.5rem" }}>
        ...
        <div className="testimonials-mobile" style={{ display: "none" }}>
          <TestCard t={TESTIMONIALS[active]} isCenter={true} onClick={() => {}} />
          {/* Nav row */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 20 }}>
            <button onClick={prev} style={{
              width: 40, height: 40, borderRadius: "50%", border: "1.5px solid rgba(13,21,32,0.12)",
              background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 3L5 8l5 5" stroke="#0D1520" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
            <div style={{ display: "flex", gap: 6 }}>
              {TESTIMONIALS.map((_, i) => (
                <button key={i} onClick={() => go(i)}
                  style={{ width: i === active ? 20 : 6, height: 6, borderRadius: 3, border: "none", cursor: "pointer", background: i === active ? "#1A50A0" : "rgba(13,21,32,0.15)", padding: 0, transition: "all 0.3s" }} />
              ))}
            </div>
            <button onClick={next} style={{
              ...
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 3l5 5-5 5" stroke="#0D1520" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
          </div>
        </div>
      </div>
