...
export default function HowItWorksSection() {
  return (
    <section id="how-it-works" style={{ background: "#FFFFFF", padding: "72px 0" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 2.5rem" }}>
        ...
        {/* Desktop: 4-col row */}
        <div className="steps-grid steps-desktop" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)" }}>
          {STEPS.map((s, i) => (
        ...
        {/* Mobile: 2x2 grid */}
        <div className="steps-mobile" style={{ display: "none", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          {STEPS.map((s) => (
            <MobileStepCard key={s.step} {...s} />
          ))}
        </div>
      </div>
      <style>{`
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: hovered ? 1 : 0, position: "relative",
      }}>
...
function MobileStepCard({ step, title, desc }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Cormorant',serif", fontWeight: 700, fontSize: 32, lineHeight: 1, color: "rgba(26,80,160,0.15)", marginBottom: 8, letterSpacing: -1 }}>{step}</div>
      <h3 style={{ fontFamily: "'Cormorant',serif", fontWeight: 700, fontSize: 16, color: "#0D1520", marginBottom: 5, lineHeight: 1.2 }}>{title}</h3>
      <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: "rgba(13,21,32,0.48)", lineHeight: 1.6 }}>{desc}</p>
    </div>
  );
}
