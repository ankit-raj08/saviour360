...
const toIndustry = (doc) => ({
  id:        doc.slug  || doc.id,
  eyebrow:   doc.eyebrow,
  tag:       doc.tag       || null,
  tagColor:  doc.tagColor  || null,
  title:     doc.title,
  subtitle:  doc.subtitle,
  desc:      doc.desc      || doc.description || "",
  usecases:  (doc.usecases || []).map(u => u.label || u),
  stat:      doc.stat,
  statLabel: doc.statLabel,
  dark:      doc.dark      || false,
});

const INDUSTRIES_ENDPOINT = "/api/industries?where[active][equals]=true&sort=order&limit=20";

export default function Industries() {
  const { data: industries } = useCMS(INDUSTRIES_ENDPOINT, FALLBACK_INDUSTRIES, toIndustry);
