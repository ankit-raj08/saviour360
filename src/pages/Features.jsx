...
const toFeature = (doc) => ({
  ...
  points:    (doc.points   || []).map(p => p.text || p),
  stat:      doc.stat      || "",
  showEmbed: doc.showEmbed || false,
  embedSrc:  doc.embedSrc  || "",
});

const PAGE_FEATURES_ENDPOINT = "/api/page-features?where[active][equals]=true&sort=order&limit=20";
...
export default function Features() {
  const { data: features } = useCMS(PAGE_FEATURES_ENDPOINT, FALLBACK_FEATURES, toFeature);
